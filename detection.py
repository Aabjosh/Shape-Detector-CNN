...
while True:
    ...
    if keyPress == 27:
        ...
        input_tensor = preprocess(pil_img).unsqueeze(0).to(device) # unsqueeze adds a batch number of 1 with '.unsqueeze(0)', so that the other code from 'model.py' can be used to run the detection model on our frame
